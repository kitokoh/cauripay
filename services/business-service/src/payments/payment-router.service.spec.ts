...
import { RailRegistry } from '@cauripay/payment-rail-contracts';
import type { IRailAdapter, RailPayment } from '@cauripay/payment-rail-contracts';
...
describe('PaymentRouterService', () => {
  ...
  it('route vers le rail demandé', async () => {
    const result = await router.initiate({ id: 'p1', amountMinor: 100, currency: 'XAF', method: 'mobile_money' }, 'GOURSI');
    expect(result.rail).toBe('GOURSI');
    expect(result.status).toBe('SUCCEEDED');
  });

  it('route par défaut vers GOURSI', async () => {
    const result = await router.initiate({ id: 'p2', amountMinor: 100, currency: 'XAF', method: 'mobile_money' });
    expect(result.rail).toBe('GOURSI');
  });
  ...
  it('retourne FAILED si le rail jette', async () => {
    ...
    const result = await router.initiate({ id: 'p3', amountMinor: 100, currency: 'XAF', method: 'x' }, 'BROKEN');
    expect(result.status).toBe('FAILED');
  });

  it('accepte un rail factice sans modifier le router (extensibilité)', async () => {
    registry.register(new GimacpayRailAdapter());
    const result = await router.initiate({ id: 'p4', amountMinor: 100, currency: 'XAF', method: 'x' }, 'GIMACPAY');
