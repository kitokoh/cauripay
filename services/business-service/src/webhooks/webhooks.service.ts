...
export class WebhooksService {
  ...
  private static readonly BACKOFF_SECONDS = [1, 5, 30, 300];
  ...
  async dispatch(merchantId: string, event: string, payload: Record<string, unknown>): Promise<WebhookDelivery> {
    const endpoint = [...this.endpoints.values()].find(
      (e) => e.merchantId === merchantId && e.active && (e.events.includes('*') || e.events.includes(event)),
    );
    if (!endpoint) {
