...
describe('WebhooksService', () => {
  ...
  it('enregistre un endpoint avec secret', () => {
    const e = service.register({ merchantId: 'm1', url: 'https://app.example.com/hooks', events: ['*'] });
    expect(e.secret).toHaveLength(32);
    expect(e.active).toBe(true);
  });

  it('signe avec HMAC-SHA256 (t + payload)', () => {
    const e = service.register({ merchantId: 'm1', url: 'https://x.example', events: ['payment.succeeded'] });
  ...
  it('dispatche après SUCCESS', async () => {
    ...
    const d = await service.dispatch('m1', 'payment.succeeded', { paymentId: 'p1', amount: 100, status: 'SUCCEEDED' });
