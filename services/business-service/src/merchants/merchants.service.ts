...
export class MerchantsService {
  ...
  createPaymentRequest(merchantId: string, dto: { amountMinor: number; currency: string; reference: string }): MerchantPayment {
    if (dto.amountMinor <= 0) {
      throw new UnprocessableEntityException({ code: 'INVALID_AMOUNT', message: 'Montant invalide' });
    }
