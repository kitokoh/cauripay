...
export type BulkStatus = 'DRAFT' | 'PENDING_APPROVAL' | 'APPROVED' | 'PROCESSING' | 'COMPLETED' | 'FAILED';
...
export class BulkService {
  ...
  private static readonly HEADER = ['destinataire', 'telephone', 'montant', 'reference'];
  ...
  upload(merchantId: string, userId: string, csvText: string): BulkPayment {
    ...
    const lines: BulkLine[] = rawLines.slice(1).map((line, idx) => this.validateLine(idx + 2, line));
    const elapsed = Date.now() - start;

    if (lines.length > BulkService.MAX_LINES) {
      throw new ConflictException({ code: 'BULK_TOO_LARGE', message: `Max ${BulkService.MAX_LINES} lignes` });
    }
    if (elapsed > 5000) {
      throw new ConflictException({ code: 'BULK_TOO_SLOW', message: `Validation > 5 s (${elapsed} ms)` });
    }
  ...
  approve(id: string, checkerId: string): BulkPayment {
    ...
    if (b.createdBy === checkerId) {
      throw new ForbiddenException({ code: 'MAKER_CHECKER', message: 'Le maker ne peut pas approuver sa propre batch' });
    }
    b.status = 'APPROVED';
  ...
  exportCsv(id: string): string {
    ...
    const rows = b.lines.map((l) =>
      [l.lineNumber, l.receiverPhone, l.amountMinor, l.reference, l.valid ? 'OK' : 'ERREUR', l.errors.join('|')].join(','),
    );
