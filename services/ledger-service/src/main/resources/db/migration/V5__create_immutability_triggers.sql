-- V5 : triggers d'intégrité — immutabilité du journal + solde non négatif

-- 1) ledger_entries est physiquement immuable
CREATE OR REPLACE FUNCTION ledger.prevent_mutation()
RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'Opération interdite: % sur ledger_entries', TG_OP;
END;
$$ LANGUAGE plpgsql;
...
DROP TRIGGER IF EXISTS trg_balance_non_negative ON ledger.ledger_balances;
