...
export class TransactionsService {
   ...
   */
  async transfer(userId: string, dto: { receiverPhone: string; amountMinor: number; idempotencyKey: string }) {
    const sender = await this.prisma.user.findUnique({
      where: { id: userId },
      include: { wallets: true },
    });
    if (!sender) throw new NotFoundException({ code: 'USER_NOT_FOUND', message: 'Émetteur inconnu' });

    const receiver = await this.prisma.user.findUnique({ where: { phone: dto.receiverPhone } });
    if (!receiver) throw new NotFoundException({ code: 'RECEIVER_NOT_FOUND', message: 'Bénéficiaire inconnu' });
    ...
    const receiverWallet = (await this.prisma.wallet.findFirst({ where: { userId: receiver.id } }))!;
    ...
    const limit = this.fees.checkKyc(sender.kycLevel, dto.amountMinor + feeAmount, 0, 0, Number(balance));
    if (!limit.allowed) {
      throw new ConflictException({ code: 'KYC_LIMIT_EXCEEDED', message: `Limite KYC: ${limit.reason}` });
    }
    ...
    const ledgerResult = await this.ledger.transfer({
      ...
      type: TransactionType.P2P as unknown as import('@cauripay/shared-types').TransactionType,
  ...
  async cashIn(agentId: string, dto: { customerPhone: string; amountMinor: number; idempotencyKey: string }) {
    const customer = await this.prisma.user.findUnique({ where: { phone: dto.customerPhone } });
    if (!customer) throw new NotFoundException({ code: 'CUSTOMER_NOT_FOUND', message: 'Client inconnu' });
    ...
    await this.ledger.transfer({
      ...
      type: TransactionType.CASH_IN as unknown as import('@cauripay/shared-types').TransactionType,
  ...
  async cashOut(agentId: string, dto: { customerPhone: string; amountMinor: number; idempotencyKey: string }) {
    const customer = await this.prisma.user.findUnique({ where: { phone: dto.customerPhone } });
    if (!customer) throw new NotFoundException({ code: 'CUSTOMER_NOT_FOUND', message: 'Client inconnu' });
    const wallet = (await this.prisma.wallet.findFirst({ where: { userId: customer.id } }))!;
    ...
    await this.ledger.transfer({
      ...
      amount: String(dto.amountMinor),
      currency: wallet.currency,
      type: TransactionType.CASH_OUT as unknown as import('@cauripay/shared-types').TransactionType,
  ...
  async reverse(transactionId: string, dto: { reason: string; idempotencyKey: string }) {
    ...
    if (!tx) throw new NotFoundException({ code: 'TRANSACTION_NOT_FOUND', message: 'Transaction inconnue' });
