...
describe('ReconciliationService', () => {
  ...
  const makeSource = (
    data: Array<{ walletId: string; computed: number; stored: number }>,
  ...
  it('exporte un CSV détaillé par wallet', async () => {
    service = new ReconciliationService(
      makeSource([{ walletId: 'w1', computed: 1000, stored: 1000 }]),
    );
  ...
  it('exporte un PDF de synthèse', async () => {
    service = new ReconciliationService(
      makeSource([{ walletId: 'w1', computed: 500, stored: 500 }]),
    );
