...
paiements marchands, KYC/AML). L'agrégateur de paiement v0.1 (`legacy/`) n'est **pas** la cible :
...
```
cauripay/
├── apps/                    # Applications cibles : web-admin, web-business, mobile (à venir)
│   ├── api-mvp/             #   v0.1 sandbox aggregator (Fastify+SQLite) — HISTORIQUE (amendement §3 écart 6)
├── legacy/                  # v0.1 agrégateur (server + dashboard + docs) — HISTORIQUE, archivé
...
| 6 | Layout du v0.1 historique | **`legacy/`** (server + dashboard + docs v0.1) — hors workspaces npm, **tests conservés en CI** (job dédié, Node 22) ; ADR-002 appliqué tel que rédigé | Décision du responsable (2026-08-14) |
